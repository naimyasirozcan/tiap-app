...
function ExceptionListRow( { exception }) {
  return (
    <div className="w-full grid grid-cols-12 gap-0 p-2">
      ...
      <div className="h-[40px]col-span-1 flex items-center justify-start text-[11px]">{exception.exceptionLocation === null ? "-" : exception.exceptionLocation.name}</div>
      <div className="h-[40px]col-span-1 flex items-center justify-start text-[11px]">{exception.errorBy === null ? "-" : exception.errorBy.email.split("@")[0]}</div>
